...
--             case acc of
--                  (Full _) -> acc
--                  _ -> if p(h) then (Full h) else Empty
--   ) Empty l
find p =
  foldRight (\h acc -> if p h then Full h else acc) Empty
...
-- lengthGT4 Nil = False
-- lengthGT4 l =
--           let f c (_ :. t) = case t of
--                              Nil -> c
--                              _   -> if c > 4 then True else f (c + 1) t
--           in
--           f 1 l
...
oneToFiftyThousand :: List Int
oneToFiftyThousand = listh [1..50000]
